...
@pytest.mark.integ
@pytest.mark.asyncio
async def test_happypath_place_state(test_data):
    actual = await GeoLocator.fetch_geolocation([test_data["data1"]["city_state"]])
...
@pytest.mark.integ
@pytest.mark.asyncio
async def test_happypath_zip(test_data):
    actual = await GeoLocator.fetch_geolocation([test_data["data1"]["zipcode"]])
...
@pytest.mark.integ
@pytest.mark.asyncio
async def test_happypath_multiple_place_state(test_data):
    actual = await GeoLocator.fetch_geolocation([test_data["data1"]["city_state"], test_data["data2"]["city_state"]])
...
@pytest.mark.integ
@pytest.mark.asyncio
async def test_happypath_multiple_zip(test_data):
    actual = await GeoLocator.fetch_geolocation([test_data["data1"]["zipcode"], test_data["data2"]["zipcode"]])
...
@pytest.mark.integ
@pytest.mark.asyncio
async def test_happypath_mix_place_state_zip(test_data):
    actual = await GeoLocator.fetch_geolocation([test_data["data1"]["city_state"], test_data["data2"]["zipcode"]])
...
@pytest.mark.integ
@pytest.mark.asyncio
async def test_invalid_place_state():
    actual = await GeoLocator.fetch_geolocation(["Santa Clara, wa"])
...
@pytest.mark.integ
@pytest.mark.asyncio  
async def test_invalid_zip(caplog):
    with caplog.at_level(logging.INFO):
        await GeoLocator.fetch_geolocation(["99999"])    
...
def test_e2e_zip_not_found(app_path):
    app = os.path.abspath(app_path)
    result = subprocess.run([app, "--log", "--locations", "99999"], capture_output=True, text=True)
    assert result.returncode == 0
